...
(function ($) {
		...
		function renderOrder() {
			...
			if (!cart.length) {
				...
				$ops.html(cart.map(function (item) {
					...
					if (!prod) return '';
					return '<div class="order-col"><div>' + item.qty + 'x ' + prod.name + '</div><div>' + money(prod.price * item.qty) + '</div></div>';
				}).join(''));
			}
	...
	var INFO_PAGES = {
		...
		account:  { title: 'My Account', body: '<p>Welcome to your account area.</p><p>From here you can view your cart, check your wishlist, and track recent orders:</p><p><a href="checkout.html" class="primary-btn cta-btn">View Cart</a> <a href="store.html?wishlist=1" class="primary-btn cta-btn" style="background:#e4e7ed;color:#15161d;">My Wishlist</a> <a href="checkout.html?track=1" class="primary-btn cta-btn" style="background:#e4e7ed;color:#15161d;">Track My Order</a></p>' }
	};
	...
	function bindGlobalEvents() {
		...
		$(document).on('click', '.add-to-cart-btn', function (e) {
			...
			if (!id) return;
			var qty = 1;
			var $qtyInput = $(this).closest('.add-to-cart').find('.input-number input[type="number"]');
